...
void Renderer::initVulkan() {
    if (!glfwVulkanSupported())
        throw std::runtime_error("Cannot use vulkan !");
    createInstance();
...
TextHandler &Renderer::getTextHandler() {
    return *textHandler;
}

void Renderer::setStartTime(double start) {
...
void Renderer::createVertexBuffer(VkDeviceSize size) {
    vertexBuffer = new Buffer(*device, size,
...
void Renderer::createIndexBuffer(VkDeviceSize size) {
    indexBuffer = new Buffer(*device, size,
                             VK_BUFFER_USAGE_TRANSFER_DST_BIT
...
void Renderer::loadGuiShaders(std::string vShader, std::string gShader,
    ...
    if (!vShader.empty())
        this->guiVertShader = vShader;
    if (!gShader.empty())
        this->guiGeomShader = gShader;
    if (!fShader.empty())
        this->guiFragShader = fShader;
...
void Renderer::loadFinalShaders(std::string vShader, std::string gShader,
    ...
    if (!vShader.empty())
        this->finalVertShader = vShader;
    if (!gShader.empty())
        this->finalGeomShader = gShader;
    if (!fShader.empty())
        this->finalFragShader = fShader;
...
void Renderer::updateUniformBuffer(uint32_t currentImage) {
    ...
    if (!game.getEntities().empty())
        uniformBuffers[currentImage]->fill(p);

    if (game.getGuis().empty())
        return;
...
VkExtent2D Renderer::getFramebufferSize() {
    ...
    VkExtent2D res{};
    if (vkGetPhysicalDeviceSurfaceCapabilitiesKHR(this->physicalDevice,
                                                  this->surface, &cap)
        != VK_SUCCESS) {
        window.getFramebufferSize(&res.width, &res.height);
        return res;
    }
    res = cap.currentExtent;
    // Dumbass driver giving -1 in unsigned values
    if (res.width > cap.maxImageExtent.width
        || res.height > cap.maxImageExtent.height) {
        window.getFramebufferSize(&res.width, &res.height);
    }
    return res;
}
