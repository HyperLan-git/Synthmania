...
Accidental getAccidental(unsigned char pitch, KeySignature signature) {
    ...
    switch (key) {
        case 0:
            if (signature.sharps >= 1 || signature.sharps <= -6)
                return NATURAL;
            return NONE;
        case 1:
            if (signature.sharps >= 2 || signature.sharps <= -4)
                return NONE;
            return SHARP;
        case 2:
            if (signature.sharps >= 4 || signature.sharps <= -4)
                return NATURAL;
            return NONE;
        case 3:
            if (signature.sharps >= 4 || signature.sharps <= -2)
                return NONE;
            return SHARP;
        case 4:
            if (signature.sharps >= 6 || signature.sharps <= -2)
                return NATURAL;
            return NONE;
        case 5:
            if (signature.sharps >= 1 || signature.sharps <= -7)
                return NATURAL;
            return NONE;
        case 6:
            if (signature.sharps >= 1 || signature.sharps <= -5)
                return NONE;
            return SHARP;
        case 7:
            if (signature.sharps >= 3 || signature.sharps <= -5)
                return NATURAL;
            return NONE;
        case 8:
            if (signature.sharps >= 3 || signature.sharps <= -3)
                return NONE;
            return SHARP;
        case 9:
            if (signature.sharps >= 5 || signature.sharps <= -3)
                return NATURAL;
            return NONE;
        case 10:
            if (signature.sharps >= 5 || signature.sharps <= -1)
                return NONE;
            return SHARP;
        case 11:
            if (signature.sharps >= 7 || signature.sharps <= -1)
                return NATURAL;
            return NONE;
    }
    return NONE;
...
PartitionNotation::PartitionNotation(std::string name, int64_t time,
    ...
    this->position.y = 0.5f
        - 0.083f * (getDifferenceFromC4(pitch, signature) + getOffset(key));
}
...
int64_t PartitionNotation::getTime() {
    return time;
}
