...
#define SPAWN_TEXT_FUN(textHandler, container, fun, args...)                   \
    for (auto &g_text : textHandler.fun(args))                                 \
...
class TextHandler {
   public:
     ...
     */
    void
    loadFonts(std::map<std::string, std::vector<unsigned long>> fontsToLoad,
              CommandPool &commandPool);
    ...
    printString(std::string text, std::string entityNames, std::string font,
                double size, glm::vec2 pos, glm::vec4 color = {0, 0, 0, 1});

    std::vector<std::shared_ptr<Gui>>
    printVerticalString(std::string text, std::string entityNames,
                        std::string font, double size, glm::vec2 pos,
                        glm::vec4 color = {0, 0, 0, 1});

    std::vector<std::shared_ptr<Gui>>
    printShadowedString(std::string text, std::string entityNames,
                        std::string font, double size, glm::vec2 pos,
                        glm::vec4 color = {0, 0, 0, 1});

    std::vector<std::shared_ptr<Gui>>
    printShakingString(std::string text, std::string entityNames,
                       std::string font, double size, glm::vec2 pos,
                       float shake, glm::vec4 color = {0, 0, 0, 1});

    std::vector<std::shared_ptr<PartitionNotation>>
    printStringAsNotation(std::string text, std::string entityNames,
